...
//	static final String Master2PrivateKey = "0xe0b80216ba7b880d85966b38fcd8f7253882bb1386b68b33a8e0b60775e947c0";
    ...
    public void testGetMaster() {
        ...
        final String targetPrivateKey = "6e8ad4e4ffb888082d94975a58dc9a8179f8724ba22301cd8392ba5352af7e25";
        final String targetAddress = ECKeyPair.create(targetPrivateKey).getHexAddress();
        final ContractCallResult callResult = ProtoTypeContractClient.getMasterAddress(
                Address.fromHexString(targetAddress),
                Revision.BEST
        );
        logger.info("testGetMaster fromAddress:" + targetAddress);
    ...
    public void testSetMasterAndMasterActions() throws IOException {
        ...
        final String MasterAddress2 = "...";
        final String UserAddress = "";
        final String Master2PrivateKey = "";

        final ECKeyPair aECKeyPair = ECKeyPair.create(privateKey);
        final TransferResult result = ProtoTypeContractClient.setMasterAddress(
        ...
        final ContractCallResult callResult = ProtoTypeContractClient.getMasterAddress(
                Address.fromHexString(aECKeyPair.getHexAddress()),
                Revision.BEST
        );
        logger.info("checkMaster: " + JSON.toJSONString(callResult, prettyFormat));
        Assert.assertEquals(true, callResult.getData().contains(MasterAddress2.substring(2)));

        final Address address = Address.fromHexString(MasterAddress2);
        ...
        final TransferResult transferResult = ProtoTypeContractClient.addUsers(
                new Address[]{Address.fromHexString(fromAddress)},
        ...
        logger.info("Add user: " + JSON.toJSONString(transferResult, false));
        ...
        logger.info("Add user Receipt: " + JSON.toJSONString(receipt, prettyFormat));
        ...
        ContractCallResult isUserResult = ProtoTypeContractClient.isUser(
                ...
                Revision.BEST);
        logger.info("Get isUser result: " + JSON.toJSONString(isUserResult, prettyFormat));
        Assert.assertNotNull(isUserResult);

        final Amount credit = Amount.VTHO();
        credit.setDecimalAmount("0.1");
        final Amount recovery = Amount.VTHO();
        recovery.setDecimalAmount("0.00001");

        final TransferResult setCreditPlansResult = ProtoTypeContractClient.setCreditPlans(
                new Address[]{Address.fromHexString(fromAddress)},
                new Amount[]{credit},
                new Amount[]{recovery},
                TransactionClient.ContractGasLimit,
        ...
        logger.info("set user plans: " + JSON.toJSONString(setCreditPlansResult, prettyFormat));
        try {
            Thread.sleep(10000);
        } catch (InterruptedException e) {
        }

        final ContractCallResult getUserCreditCallResult = ProtoTypeContractClient.getUserCredit(
                Address.fromHexString(fromAddress),
                Address.fromHexString(UserAddress),
                Revision.BEST
        );
        logger.info("Get user plan result:" + JSON.toJSONString(getUserCreditCallResult, prettyFormat));

        final Amount aAmount = Amount.VTHO();
        aAmount.setHexAmount(getUserCreditCallResult.getData());
        Assert.assertEquals(true, aAmount.getAmount().toString().startsWith("0.1"));

        final TransferResult removeUserResult = ProtoTypeContractClient.removeUsers(
                new Address[]{Address.fromHexString(fromAddress)},
                new Address[]{Address.fromHexString(UserAddress)},
                TransactionClient.ContractGasLimit,
                (byte) 0x0,
        ...
        logger.info("Remove user:" + JSON.toJSONString(removeUserResult, prettyFormat));
        ...
        isUserResult = ProtoTypeContractClient.isUser(
                Address.fromHexString(fromAddress),
                Address.fromHexString(UserAddress),
                Revision.BEST
        );
        ...
        final String addressHex = ECKeyPair.create(sponsorKey).getHexAddress();
    ...
    public void testIsUser() throws IOException {
        // pre-seeded solo account[0]
        final String fromAddress = "0x3db469a79593dcc67f07DE1869d6682fC1eaf535";
        // original
        final String UserAddress = "0xc71ADC46c5891a8963Ea5A5eeAF578E0A2959779";
        final ContractCallResult callResult = ProtoTypeContractClient.isUser(
                Address.fromHexString(fromAddress),
                Address.fromHexString(UserAddress),
                Revision.BEST
        );
        logger.info("Get isUser result: " + JSON.toJSONString(callResult, prettyFormat));
    ...
    public void testAddUser() throws IOException {
        ...
        final String fromAddress = "0x3db469a79593dcc67f07DE1869d6682fC1eaf535";
        // original
        final String UserAddress = "0x2669514f9fe96bc7301177ba774d3da8a06cace4";
        final TransferResult transferResult = ProtoTypeContractClient.addUsers(
                new Address[]{Address.fromHexString(fromAddress)},
                new Address[]{Address.fromHexString(UserAddress)},
                TransactionClient.ContractGasLimit,
        ...
        logger.info("Add user: " + JSON.toJSONString(transferResult), prettyFormat);
